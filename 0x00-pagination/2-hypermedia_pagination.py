...
class Server:
    ...
    def get_hyper(self, page: int = 1, page_size: int = 10) -> dict:
        """returns a dictionary as in hypermedia"""
        assert(type(page_size) is int)
        assert(type(page_size) is int)
        assert(page > 0)
        assert(page_size > 0)
        ...
        with open('Popular_Baby_Names.csv', newline='') as f:
            ...
            pages = [[field for field in row[0].split(',')] for row in allData]
            pages.pop(0)
        total_pages = math.ceil(len(pages) / page_size)
        next_page = page + 1 if page + 1 <= total_pages else None
        previous_page = page - 1 if page - 1 > 0 else None
        data = pages[start_index: end_index]
        if end_index > len(pages):
            data = []
        info = {'page_size': len(data), 'page': page, 'data': data,
